...
import { buildRothConversionModel, ROTH_ASSUMPTION_VERSION } from "@/lib/roth-conversion-analysis";
...
export async function POST(req: Request) {
  try {
    ...
    const rothWorksheet =
      body?.rothWorksheet != null && typeof body.rothWorksheet === "object"
        ? normalizeRothWorksheet(body.rothWorksheet)
        : null;
    ...
    if (age < 60) {
      return NextResponse.json({ error: "Roth Option report is only generated for clients age 60 and older." }, { status: 400 });
    }

    const bracketRaw = String(client.federalTaxBracket || "22").replace(/%/g, "").trim();
    const intakeFederal =
      ["10", "12", "22", "24", "32", "35", "37"].includes(bracketRaw) ? bracketRaw : "22";
    ...
    if (need <= 0) {
      return NextResponse.json(
        { error: "Enter annual retirement spendable income on intake to run this Roth illustration." },
        { status: 400 }
      );
    }
    ...
    const name = `Roth_Option_${cleanText(clientDisplayName(client as { firstName?: string; lastName?: string; name?: string }) || "Client").replace(/\s+/g, "_") || "Client"}.pdf`;
